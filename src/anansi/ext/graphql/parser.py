# flake8: noqa
"""Parse GraphQL strings into anansi contexts."""
...
from .exceptions import raise_syntax_error
...
class GraphQL(object):
    ...
    def __init__(self, lexer=None, **kwargs):
        self.lexer = lexer or GraphQLLexer()
        self.tokens = self.lexer.tokens
        self.yacc = yacc.yacc(module=self, **kwargs)

    def parse(self, text, **kwargs):
        """Parse GraphQL to AST tree."""
        return self.yacc.parse(input=text, lexer=self.lexer, **kwargs)
    ...
    def p_error(self, token):
        """Raise error for given token."""
        if token is None:
            raise_syntax_error('Unexpected end of input.')
        ...
        raise_syntax_error(
            'Syntax error at {}'.format(repr(fragment)),
            token=token,
        )
